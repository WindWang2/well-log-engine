...
| FRS 需求 | 状态 | 现状 | 缺口 → 归属 |
|---|---|---|---|
| MD/TVD/TVDSS/TWT 多深度刻度 | 🟡 | SDK axis md/tvd/tvdss (`core/document.hpp:45-50`)；**Desktop 画布深度刻度尺已交付**（`depth_ruler.py`，见前）；**Epic B 推进**：SDK `scene::nice_axis_ticks` 权威刻度语义（Desktop 绑定优先 + Python 回退，parity 测试锁死，消除双实现漂移）；`TimeDepthRelationship` 真 TWT domain 模型（显式 time-depth 映射、正逆变换、单位/domain/provenance 显式、空=unavailable 返回 NaN 不冒充 TVD；`time_depth_test.cpp`）；TWT 接入 depth transform 链 / 多轴同时显示 / 单井 PDF 刻度统一 → 后续切片 | TWT 链接入、多轴显示、单井 PDF 刻度 → SDK(渲染) |
