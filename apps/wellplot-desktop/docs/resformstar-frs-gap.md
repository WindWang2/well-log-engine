...
| FRS 需求 | 状态 | 现状 | 缺口 → 归属 |
|---|---|---|---|
| MD/TVD/TVDSS/TWT 多深度刻度 | 🟡 | SDK axis md/tvd/tvdss (`core/document.hpp:45-50`)；**Desktop 画布深度刻度尺已交付**：section/correlation 画布左侧刻度条（`depth_ruler.py`：`nice_depth_ticks` 1/2/2.5/5×10^k 阶梯 + `format_depth_label` + `paint_depth_ruler`；随深度窗口平移/缩放联动，correlation 含 VE 一致映射；`RULER_WIDTH=52` 预留左边距，导出/预览同一 paint 路径） | TWT 无；多轴同时显示 → SDK(渲染) |
