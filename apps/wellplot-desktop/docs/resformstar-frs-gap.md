...
| 斜井 TVD 剖面 / 沿轨迹展布 (Unfolded) | ✅ | `survey.py` 最小曲率法 + datum tvd 模式 + **地理井距展布** + **Unfolded（沿 MD 展布）已交付**（详见交接）；**真 TST 已交付（平面/局部模型，Epic D 切片 1）**：`scene/tst.hpp` `tst_from_measured_interval`（TST = L·|w·n|）/`tst_from_interval_points`（|Δp·n|），ThicknessKind 显式区分 MD/TVD/apparent/TST，单元向量校验（缺失方向=输入错误，不假装平面永远成立），零 TST 合法（井平行层面）；解析几何测试 9 组（直井/斜井 × 水平/倾斜层面、平行、极端倾角、非法输入） | **P1(计算+datum+展布+Unfolded✅)/Epic D(平面模型✅)**；复杂层面/变倾角 TST 后续 |
