...
| FRS 需求 | 状态 | 现状 | 缺口 → 归属 |
|---|---|---|---|
| MD/TVD/TVDSS/TWT 多深度刻度 | 🟡 | SDK axis md/tvd/tvdss (`core/document.hpp:45-50`)；**Desktop 画布深度刻度尺已交付**；**Epic B 推进**：`scene::nice_axis_ticks` 权威刻度语义（Desktop 绑定优先+回退，parity 锁死）+ `format_axis_tick_label`（标签精度/漂移同规则）；`TimeDepthRelationship` 真 TWT domain（显式 time-depth 映射、正逆、ms/s、provenance、空=unavailable 返 NaN）；**多轴域刻度数学** `ticks_for_reference_window`/`ticks_for_twt_window`（显示窗口逆映射回参考/时间域跑权威阶梯）；**单井 SVG/PDF 导出深度刻度**（`ExportPageSpec.show_depth_ruler`，两后端同一几何/语义，连续页+分页均支持） | 多轴同时显示于画布（Desktop 消费多轴刻度 API）、Desktop 单井引擎导出启用 ruler 选项 → 后续 |
...
| 斜井 TVD 剖面 / 沿轨迹展布 (Unfolded) | ✅ | `survey.py` 最小曲率法 + datum tvd 模式 + **地理井距展布**（井列按测斜闭合位移投影摆放 + 井内弯曲轨迹线，`well_spacing="geographic"`）+ **Unfolded（沿 MD 展布）已交付**：`well_spacing="unfolded"`，曲线数据沿井筒轨迹弯曲绘制（`path_segment_frame` 按显示深度插值路径 + 所在段方向，画布按像素空间法向偏移成 corridor 样式；无测斜井回退垂直柱；datum shifts 随路径，深度窗口按位移适配）；**顺带修复**：geographic/unfolded 无论 datum 模式都加载测斜（此前仅 tvd/tvdss 加载，geographic+md 静默退化为等井距）；combo 增「Unfolded（沿 MD 展布）」项；真 TST 仍缺 | **P1(计算+datum+展布+Unfolded✅)/后续**: 真 TST |
