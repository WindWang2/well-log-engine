...
| 试油/解释成果道 | ✅ | **Epic C 已交付**（Desktop host 画布 + 导出共用）：新 track role `well_test`（`template_model`/`display_set` 绑定），`paint_well_test_track`（`multi_track_canvas.py`，导出同 helper）按流体着色带（油/气/水/油水同层…）+「类型·结论」标签 + 宽道解释成果行；`WellTestDialog` 编辑器；内置图版「标准工程综合图」（`templates/std_engineering.json`：深度+GR+试油+射孔）；shell 保存后重应用图版即时刷新 | **Epic C(试油道✅)**；TVD/TVDSS 域区间投影后续（现按显示深度轴渲染，见 Epic B 多轴路径） |
| 射孔/井下工程道 | ✅ | **Epic C 已交付**（Desktop host 渲染）：新 track role `perforation`，`paint_perforation_track` 状态着色带（已射/已封堵/补射/未射）+ 每孔一点（孔密×厚度，上限 40 点，未知密度默认 5 点）+ 状态/孔密标签；`PerforationDialog` 编辑器；SDK marker `casing_shoe`（`core/document.hpp:433-439`）符号语义仍为 SDK 侧后续 | **Epic C(射孔道✅)**；SDK 侧符号渲染后续 |
