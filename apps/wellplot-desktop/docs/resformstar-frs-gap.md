...
| 统一地层层序字典（界-系-统-组-段-小层-砂层 + 颜色/线型/花纹） | ✅ | **Epic C 已交付**：`stratigraphy.py` 工区级 `StratigraphicDictionary`（稳定 id/parent/name/code/颜色/线型/花纹/地质时代/显式排序/source/version；标准八级为可扩展常量非封闭枚举；层级导航 root/children/path_to + 环/孤儿/重复校验；`stratigraphy.json` 工区级边车持久化）；`FormationTop.unit_id` 可选引用统一单元（旧 tops.json 无字段照常读取，无 schema bump）；Core/试油/射孔域均经 unit_id 引用层序 | **Epic C(层序字典✅)** |
| 岩心/试油/射孔数据库 | ✅ | **Epic C 已交付（数据模型，UI 后续）**：`core_model.py`（CoreRun 顶/底/取心/样品 + 物性孔渗密 + 化验引用 + 照片段 id 关联——照片保持 CorePhotoModel 所有权；`wells/<id>/core.json`）；`well_test_model.py`（结构化公共字段 + 类型特有 typed payload 可扩展 schema；`wells/<id>/well_test.json`）；`perforation_model.py`（射孔区间/密度/相位/状态/完井引用；`wells/<id>/perforation.json`）；C5 全部深度对象显式 depth+unit+depth_domain；层位统一经层序字典引用 | **Epic C(数据模型✅)**；道渲染（试油/解释成果道、射孔/井下工程道）留后续 |
